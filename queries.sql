...
JOIN departments d
    ON e.department_id = d.department_id
GROUP BY d.name
HAVING AVG(e.salary) > (SELECT overall_avg_salary FROM company_avg)
...
('Google Data Analytics', 'Google', 'Associate'),
('Scrum Master', 'Scrum Alliance', 'Professional');
...
ORDER BY e.first_name, e.last_name, ec.certification_date;