...
-- Tier 3 Challenge 4: Production migration analysis
-- In production, I would add the new salary_history table first without removing
-- the existing salary column from employees. Then I would backfill data in batches,
-- validate row counts and salary totals, and test the migration in a staging
-- environment before cutover. The main risks are duplicate records, missing rows,
-- incorrect effective dates, and live data changing during the backfill process.
-- To reduce risk, I would use transactions where possible, take backups, validate
-- with comparison queries, and switch application writes carefully after verification.